...
С помощью minikube разверните у себя локально кластер kubernetes, а внутри запустите [контейнер](https://hub.docker.com/r/kholstinevich/server).
...
Также можно сделать деплой через dashboard, для этого используйте команду `minikube dashboard`.
...
У minikube есть различные [драйверы](https://minikube.sigs.k8s.io/docs/drivers/) для запуска (опция `--driver`), рекомендую использовать `docker` (необоходимо чтобы docker desktop был запущен).